...
| **配置** | 全部 36 个配置项（含卡片宽度、封面裁剪、缓存清理间隔、调试日志、媒体发送） |
...
全部 36 项，分 8 组，常用在前。这 8 组与页面里的分组一一对应。
...
| 分享卡片渲染（`core/card_renderer.py`） | ✅ 4 布局 × 深浅双主题，可自定义强调色 / 渐变 / 水印 / 正文行数 |
...
| 配置（`_conf_schema.json` + `core/config.py`） | ✅ 8 组 36 项，页面内维护、保存即生效 |
