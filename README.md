...
| **配置** | 全部 38 个配置项（含卡片宽度、封面裁剪、缓存清理间隔、调试日志、媒体发送、HTTPS 证书校验） |
...
具体路径在网页界面「总览 → 自定义解析器」那张卡上直接写着，点「打开目录」的提示也取自那里。
...
3. 回到网页界面「总览 → 自定义解析器」，点 **「重新加载」** —— 立刻生效，不用重启插件
...
平台键不能与内置的 11 个重名（`bilibili` / `douyin` / `kuaishou` / `weibo` /
